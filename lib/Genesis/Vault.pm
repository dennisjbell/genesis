	...
	if ($action =~ /^(remove|recreate|renew)$/ && !$opts{'no-prompt'}) {
		...
		my $permission = $update->('prompt',
			...
			msg => "The following secrets will be ${actioned}:\n  ".
			       join("\n  ", map {bullet $_, inline => 1} map {_get_plan_paths($_)} @plans),
...
sub describe_kit_secret_plan {
	...
	if ($plan{type} eq 'x509') {
		...
		@features = (
			$plan{is_ca} ? 'CA' : undef,
			$plan{self_signed} ? 'self-signed' : $plan{signed_by} ? "signed by '$plan{signed_by}'" : undef
		);
...
sub _generate_secret_command {
	...
	if ($action eq 'remove') {
		...
		if ($action_map{$action} eq 'issue') {
			...
			push(@cmd, '--key-usage', $_) for (@{$plan{usage} || []});
		}
	} elsif ($plan{type} eq 'random') {
		@cmd = ('gen', $plan{size},);
	...
	push(@cmd, '--no-clobber') if $action eq 'add' || $plan{fixed};
...
sub _process_x509_plans {
	...
	_sign_x509_plans_with_root_ca($signers->{''}, $plans, $root_ca_path )
		if ($root_ca_path);
...
# }}}
# _sign_x509_plans_with_root_ca {{{
sub _sign_x509_plans_with_root_ca {
	...
	if ($signer) {
		if (! grep {$_->{path} eq $signer} (@$ordered_plans)) {
			...
			if (defined($idx)) {
				...
				$src_plans->{$signer}{self_signed} = 1;
...
sub _validate_x509_plan {
	...
	if ($cert{usage}) {
		if (CORE::ref($cert{usage}) eq 'ARRAY') {
			if (! scalar @{$cert{usage}}) {
				$err .= "\n- Invalid usage argument: expecting an array of one or more strings, got an empty list";
			} else {
...
sub _validate_kit_secret {
	...
	my $errors = join("\n", map {sprintf("missing key ':%s'", $_)} grep {! exists($values->{$_})} @keys);
	...
	if ($plan->{type} eq 'x509') {
		...
		my @extra_sans = grep {!exists $desired_sans{$_}} CORE::keys %sans;
		my @missing_sans = grep {!exists $sans{$_}} CORE::keys %desired_sans;
		$results{extra_sans} = !scalar(@extra_sans);
		$results{missing_sans} = !scalar(@missing_sans);
		...
		if ($self_signed) {
			...
			if ($ca_cert) {
				...
				if ($caSubjectKeyID) {
					...
					} while ($ca_plan && $signer_path && defined($secret_values->{$signer_path}));
					my $out = run(
		...
		if ($since_t < $now_t) {
			if ($now_t < $expires_t) {
				$valid_str = sprintf("expires in %s days (%s)", int(($expires_t - $now_t)->days()), $expires);
			} else {
				$valid_str = sprintf("expired %s days ago (%s)", int(($now_t-$expires_t)->days()));
			}
		} else {
		...
		$msg .= sprintf("%sCA Certificate\n",      _checkbox($is_ca)) if $plan->{is_ca};
		$msg .= $self_signed ?
		...
		$msg .= sprintf("%sSubject Alt Names%s\n", _checkbox($results{extra_sans} && $results{missing_sans}), $sans_str);
...
sub _x509_key_usage {
	...
	my %keyUsageLookup = (
		...
		"Content Commitment" => "content_commitment", # synonym for non_repudiation
	...
	return (values %keyUsageLookup, values %extendedKeyUsageLookup)
	...
	return CORE::keys(%found) unless @check;
	...
	my @extra   = (grep {$found{$_} > 0} CORE::keys %found);
	my @missing = (grep {$found{$_} < 0} CORE::keys %found);
...
sub _checkbox {
	return bullet($_[0] ? 'good' : 'bad', '', box => 1, inline => 1);
}
# }}}
# }}}
1;
