...
sub again {
	...
	# git init it
	system("cd $tmp/test-genesis-kit && git init >/dev/null 2>&1 && git add . && git commit -m 'committed' > /dev/null 2>&1");
}

sub commit_changes {
	system("(cd $tmp/test-genesis-kit && git add -u && git commit -m 'commited changes') >/dev/null");
}
...
again();
system("rm -rf $tmp/test-genesis-kit/kit.yml");
commit_changes;
$out = combined_from {
...
EOF
system("mkdir $tmp/test-genesis-kit/ci") unless -d "$tmp/test-genesis-kit/ci";
system("mkdir -p $tmp/test-genesis-kit/hooks/info");
put_file("$tmp/test-genesis-kit/ci/test_params.yml", <<EOF);
---
params:
...
unlink "$tmp/test-genesis-kit/hooks/new" if -f "$tmp/test-genesis-kit/hooks/new";
...
system("(cd $tmp/test-genesis-kit && git add hooks && git commit -m 'commited changes') >/dev/null");
system("(cd $tmp/test-genesis-kit && git add ci && git rm -f hooks/blueprint) >/dev/null");

put_file("$tmp/test-genesis-kit/hooks/check", <<EOF);
#!/bin/bash
set -ue
exit 1
EOF
